...
<main>
  <section class="exercises-list">
    <?php
    $dir = 'uploads/exercise'; // Adjust the path to your JSON files directory

    $jsonFiles = glob($dir . '/*.json');

    foreach ($jsonFiles as $file) {
        $jsonContent = file_get_contents($file);
        $exercise = json_decode($jsonContent, true); // Assuming each file contains a single lesson object

        echo '<div class="exercise">';
        echo '<img src="' . htmlspecialchars($exercise['imagePath']) . '" alt="' . htmlspecialchars($exercise['title']) . '">';
        echo '<div class="exercise-content">';
        echo '<h2 class="exercise-title">' . htmlspecialchars($exercise['title']) . '</h2>';
        echo '<p class="exercise-summary">' . htmlspecialchars($exercise['content']) . '</p>';
        echo '</div></div>';
    }
    ?>
  </section>
